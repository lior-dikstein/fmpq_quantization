...
def weight_lfh(in_images, in_model, hconfig: HessianConfig = HessianConfig(), quant_only=False):
...
def set_model_hessian_scores(model, in_images, n_iter=1e-4, quant_only=False):
    hc = HessianConfig(n_iter=n_iter)
    results = weight_lfh(in_images, model, hc, quant_only=quant_only)
    for n, m in tqdm(model.named_modules()):
        if isinstance(m, CompressionWrapper) and quant_only:
