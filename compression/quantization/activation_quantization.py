...
def hook_fp_act(q_model, calib_data, args):
    ...
    # folder_path = f"/data/nobackup/cache_nobackup/users/liord/fp_output/{args.model_name}-W{args.weight_n_bits}A{args.activation_n_bits}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ###########################################
    ...
    for n, m in q_model.named_modules():
        ...
        if isinstance(m, CompressionWrapper):
            if not m.is_conv:
                ...
                quan_output = last_act[-1](store_input[0].cuda())
                del store_input
